...
var text, button, context, state = 0;
...
function tap() {
    if(state == 0) {
        ...
        for(var i = 0; i < 100; i++) {
            beep(1750, context.currentTime + i);
        }
        record();
    } else {
        if(context.currentTime > last_tap + 0.2) {
            log_tap.push(context.currentTime);
            if(log_beep.length > 0 && log_tap.length > 0)
                text.innerHTML = log_beep[log_beep.length-1] + ' ' + log_tap[log_tap.length-1];
        }
        ...
        function iteration() {
            ...
            if(features[0] > 150 && features[1] > 3) {
                ...
                if(context.currentTime > last_beep + 0.1) {
                    ...
                    if(log_beep.length > 0 && log_tap.length > 0)
                        text.innerHTML = log_beep[log_beep.length-1] + '<br>' + log_tap[log_tap.length-1];
                }
